...
# 12.定时器设计
1.为了提升定时器效率，定时器链表使用定时递增的方式组织排列
2.实现了定时器的初始化
3.定时器的增加，移除和扫描(扫描定时器时，只需要扫描表头部分即可)
4.让工作线程处理定时器事件

# 13.ARP协议
![ARP协议中的网络层](docs/Snipaste_2025-10-12_20-38-13.png)
1. ARP协议在转换IP地址时,先向网络上发送一个以太网广播，请求拥有IP地址的主机回应,在收到相应的回应之后，从中提取出硬件地址
![ARP操作](docs/Snipaste_2025-10-12_20-51-56.png)
2. 为了避免每次都需要,先将arp缓存,生成一个缓存表，需要再去取,ARP缓存表:使用链表进行排序,使得最频繁使用的表项能处于表头能被尽快查询获得
3. 由于计算机可能由于各种原因退出，所以IPv4地址可能分配给不同计算机，ARP缓存表中的表项必须设置有效期，以便保持教新的状态
![ARP数据包格式](docs/Snipaste_2025-10-12_21-29-03.png)
